...
Credit to Dr Barnaby Martin,
lecturer for AI Search submodule.
"""
    ...
    if flag[0] != "e":
        ...
        if start_of_size == -1:
            ...
            if comma_after_size == -1:
                ...
                w = charnum(d[start_of_size + 5:comma_after_size])  # get number of cities
                n = int(w)
...
for eachitem in os.listdir('.'):
    if os.path.isdir(eachitem):  # check that an item is a directory
        ...
        if interested_dir_list != []:  #look to see if I'm interested in this directory
            ...
            count = 0
            while count < num_interested_dir and interested is False:
                test_dir = interested_dir_list[count]
                if test_dir == eachitem:
                    interested = True
                count = count + 1
        if interested is True:  # true if this is an interesting directory
            to_ignore = False
            if ignore_dir_list != []:  # look to see if I'm disinterested in this directory
                num_disinterested_dir = len(ignore_dir_list)
                count = 0
                while count < num_disinterested_dir and to_ignore is False:
                    test_dir = ignore_dir_list[count]
                    if test_dir == eachitem:
                        to_ignore = True
                    count = count + 1
            if to_ignore is False:  # false if this is an interesting directory that I'm not disinterested in
                ...
                if os.path.isdir(eachitem):   # check that an item is a directory
                    ...
                    while count < num_interested_dir and interested is False:
                        ...
                        if test_dir == eachitem:
                            interested = True
                        count = count + 1
                    if interested is True:  # true if I'm interested in this tour file directory
                ...
                for eachdir in subdirlist:
                    ...
                    for city_file in cityfilelist:
                        if not os.path.isfile('tour' + city_file):  # look to see if there is a tour file for a city file
                            ...
                            if tour_length != 0:
                                if city_file not in persons_tour:
